...
export const cleanupStaleSessions = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await Promise.all(
      stale.map((session) =>
        ctx.db.patch("sessions", session._id, {
          status: "failed",
          endedAt: Date.now(),
        }),
      ),
...
export const reconcileWebhookRetries = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await Promise.all(
      stale.map((event) =>
        ctx.db.patch("webhookEvents", event._id, {
          status: "failed",
